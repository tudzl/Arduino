"""
Sensor Monitor V3.2 - Universal CO2 + ENV Monitor
Author: Zell
Date: 2026/06/15
...
WINDOW_TITLE = "Sensor Monitor V3.2 — CO2 + ENV (S3/C3)"
...
def fetch_sea_level_pressure(lat=LOCATION_LAT, lon=LOCATION_LON):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "SensorMonitor/3.2"})
...
class SensorMonitorV3:
        ...
        try:
            ...
            if self.ax_press is not None:
                ...
                if self.pressure_values:
                    ...
                    self.ax_press.plot(ts, list(self.pressure_values), "#4CAF50", linewidth=1.5, marker=".", markersize=3)
    ...
    def _show_about(self):
        ...
        ttk.Label(frame, text="Sensor Monitor V3.2", font=("Segoe UI", 13, "bold")).pack(pady=(0, 8))
        info_lines = [
            ...
            while self.ble_running or not hasattr(self, '_closing'):
                ...
                if success:
                    self.root.after(0, self._log,
                        f"Sea-level pressure updated: {_sea_level_pressure_hpa:.2f} hPa ({_sea_level_pressure_source})")
                else:
                    self.root.after(0, self._log,
                        f"Sea-level pressure update failed, using: {_sea_level_pressure_hpa:.2f} hPa")
...
if __name__ == "__main__":
    ...
    print(f"Fetching real-time sea-level pressure from Open-Meteo...")
    if update_sea_level_pressure():
        print(f"  [OK] Sea-level pressure: {_sea_level_pressure_hpa:.2f} hPa (source: {_sea_level_pressure_source})")
    else:
        print(f"  [FAIL] Could not fetch online data. Using default: {_sea_level_pressure_hpa:.2f} hPa")
