"""
Sensor Monitor V3.9 - Universal CO2 + ENV Monitor
Author: Zell
Date: 2026/06/19
...
WINDOW_TITLE = "Sensor Monitor V3.9 — CO2 + ENV (S3/C3)"
...
# Chart line color palette (12 common colors, no white)
CHART_COLOR_PALETTE = {
    "Red": "#F44336",
    "Deep Orange": "#FF5722",
    "Orange": "#FF8C00",
...
CHART_COLOR_NAMES = list(CHART_COLOR_PALETTE.keys())
...
class SensorMonitorV3:
    ...
    def _update_charts(self):
        try:
            ...
            if has_both_devices:
                ...
                self.ax_temp.plot(ts_s3, list(self.s3_temp_values), self.chart_colors["temp_s3"], linewidth=1.5, marker=".", markersize=3, label="S3")
                ts_c3 = list(self.c3_timestamps)[-len(self.c3_temp_values):]
                self.ax_temp.plot(ts_c3, list(self.c3_temp_values), self.chart_colors["temp_c3"], linewidth=1.5, marker="s", markersize=3, label="C3")
            ...
            if has_both_hum:
                ...
                self.ax_hum.plot(ts_s3, list(self.s3_hum_values), self.chart_colors["hum_s3"], linewidth=1.5, marker=".", markersize=3, label="S3")
                ts_c3 = list(self.c3_timestamps)[-len(self.c3_hum_values):]
                self.ax_hum.plot(ts_c3, list(self.c3_hum_values), self.chart_colors["hum_c3"], linewidth=1.5, marker="s", markersize=3, label="C3")
            ...
            if self.ax_press is not None:
                ...
                if has_both_press:
                    ...
                    self.ax_press.plot(ts_s3, list(self.s3_pressure_values), self.chart_colors["press_s3"], linewidth=1.5, marker=".", markersize=3, label="S3")
                    ts_c3 = list(self.c3_pressure_timestamps)[-len(self.c3_pressure_values):]
                    self.ax_press.plot(ts_c3, list(self.c3_pressure_values), self.chart_colors["press_c3"], linewidth=1.5, marker="s", markersize=3, label="C3")
            ...
            if self.ax_alt is not None:
                ...
                if has_both_alt:
                    ...
                    self.ax_alt.plot(ts_s3, list(self.s3_altitude_values), self.chart_colors["alt_s3"], linewidth=1.5, marker=".", markersize=3, label="S3")
                    ts_c3 = list(self.c3_pressure_timestamps)[-len(self.c3_altitude_values):]
                    self.ax_alt.plot(ts_c3, list(self.c3_altitude_values), self.chart_colors["alt_c3"], linewidth=1.5, marker="s", markersize=3, label="C3")
    ...
    def _show_settings(self):
        ...
        color_frame.grid(row=13, column=0, columnspan=2, sticky=tk.EW, padx=5)
        ...
        for idx, (label_text, key) in enumerate(color_labels):
            ...
            current_name = CHART_COLOR_HEX_TO_NAME.get(current_hex, "Green")
            var = tk.StringVar(value=current_name)
            color_vars[key] = var
            combo = ttk.Combobox(color_frame, textvariable=var, values=CHART_COLOR_NAMES,
                                 state="readonly", width=12)
            combo.grid(row=row_idx, column=col_offset + 1, sticky=tk.W, padx=(2, 8), pady=2)

        # Buttons row
        btn_frame = ttk.Frame(frame)
        btn_frame.grid(row=14, column=0, columnspan=2, pady=(8, 0))
        ...
        def _apply():
            ...
            for key, var in color_vars.items():
                name = var.get()
                if name in CHART_COLOR_PALETTE:
    ...
    def _show_about(self):
        ...
        ttk.Label(frame, text="Sensor Monitor V3.9", font=("Segoe UI", 13, "bold")).pack(pady=(0, 8))
        info_lines = [
