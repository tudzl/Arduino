/* this code is working as APP launcher compatible
    ...
    Version 1.7  improve GUI, added RGB(0-255 value display)
    Version 1.6  improve autorange  2020.3.11
...
uint8_t RGB_R,RGB_G,RGB_B;//convert to RGB 888 values
...
uint16_t low_light_thre = 6000 ;
uint16_t mid_light_thre = 30000 ;
...
void setup(void) {
  ...
  M5.Lcd.println("   TCS34725 APP V1.7");
...
void loop(void) {
  //light intensity check based on Clear channel
  ...
  float RGB_A=r^2 + g^2 + b^2; 
  RGB_R = uint8_t( 255.0*((float)(r^2)/(float)RGB_A)); 
  RGB_G = uint8_t( 255.0*((float)(g^2)/(float)RGB_A)); 
  RGB_B = uint8_t( 255.0*((float)(b^2)/(float)RGB_A)); 
  //light intensity check based on lux_tao
  if (lux_tao < low_light_thre)  {
    //tcs.setIntegrationTime(TCS34725_INTEGRATIONTIME_700MS); // for low light
    lux_range = 1 ;
  }
  else if (lux_tao < mid_light_thre) {
  ...
  //RGB888
   Serial.print("R(0-255): "); Serial.print(RGB_R, DEC); Serial.print(" ");
  Serial.print("G(0-255): "); Serial.print(RGB_G, DEC); Serial.print(" ");
  Serial.print("B(0-255): "); Serial.print(RGB_B, DEC); Serial.print(" ");
  Serial.println(" ");
  if (TCS_sensor_ok) {
    ...
    if ( (r < 1000) && (g < 1000) && (b < 1000))
      M5.Lcd.printf("B:%d      \r\n",  b);
    else if ( (r > 10000) && (g > 10000) && (b > 10000))
      M5.Lcd.printf("B:%d \r\n",  b);
    else
      M5.Lcd.printf("B:%d  \r\n",  b);
    ...
    //RGB888  values
     M5.Lcd.setTextColor(RED, BLACK);
    M5.Lcd.printf("R:%3d ", RGB_R);
    M5.Lcd.setTextColor(GREEN, BLACK);
    M5.Lcd.printf("G:%3d ", RGB_G);
    M5.Lcd.setTextColor(BLUE, BLACK);
    M5.Lcd.printf("B:%3d  (0-255)\r\n", RGB_B);
//    if ( (RGB_R < 100) && (RGB_G < 100) && (RGB_B < 100))
//      M5.Lcd.printf("B:%d (0-255)  \r\n", RGB_B);
//    else if ( (RGB_R > 100) || (RGB_G > 100) || (RGB_B > 100))
//      M5.Lcd.printf("B:%d (0-255) \r\n",  RGB_B);
//    else
//      M5.Lcd.printf("B:%d (0-255)  \r\n",  RGB_B);
    M5.Lcd.setTextColor(WHITE, BLACK);
