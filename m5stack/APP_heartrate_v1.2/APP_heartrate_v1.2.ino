//Original code is for M5StickCPlus, try to be used for core2/fire
//V1.2
//V1.1 by zell
//19.Oct.2022
...
//const byte Button_A = 39;
const byte pulseLED = 26; //PORT B
...
void callBack(void)
{
  V_Button = digitalRead(BUTTON_A_PIN); 
  if(V_Button == 0)
    flag_Reset = 1;
  delay(10);
}
...
{
  //init
  M5.begin();                 //initialize Fire / M5StickCPlus
  M5.Lcd.setBrightness(50);  //define BLK_PWM_CHANNEL 7  PWM
  M5.Lcd.setTextSize(2);
  M5.Lcd.setTextColor(WHITE, BLACK);
  M5.Lcd.println(" Heart APP V1.2 Zell");
  ...
  //Wire.begin(0,26);           //initialize I2C StickC
  
  Serial.println("Smart device/M5stack | MAX30102 heart rate+SpO2 demo V1.2 ");
  Serial.println("=========================");
  M5.Lcd.setTextColor(GREEN, BLACK);
  M5.Lcd.setCursor(0, 200);
  M5.Lcd.println("MAX3010x Sensor init...");
  delay(500);
  Wire.begin();         //Fire iic port A 21/22
  //set Lcd
  //M5.Lcd.setRotation(3);
  //M5.Lcd.setSwapBytes(false);
//  Disbuff.createSprite(320, 240); //ori 240  135 for stickC
...
//  Disbuff.createSprite(320, 240);
  ...
  if(!Sensor.begin(Wire, I2C_SPEED_FAST))
  {
    ...
    M5.Lcd.println("MAX30102 Init Failed");
    while(1);
  }
  else {
    M5.Lcd.println("MAX30102 Init OK!");
  }
  Serial.println(F("Place your index finger on the Sensor with steady pressure"));
  
  attachInterrupt(BUTTON_A_PIN, callBack, FALLING);
  //set Max30102
  Sensor.setup();
  //Sensor.clearFIFO();
}
...
void loop() 
{
  ...
  while(flag_Reset == 0)
  {
    ...
    if(num_fail < 10)
    { 
      Serial.printf("spo2:%d%%,", spo2);  
      Serial.printf("BPM:%d ", beatAvg);  
      ...
      M5.Lcd.printf("Spo2: %d%% \r\n", spo2);

      M5.Lcd.setTextColor(YELLOW, BLACK);
      M5.Lcd.setTextSize(3);
      M5.Lcd.setCursor(30, 120);
      M5.Lcd.printf("Finger Present!\r\n"); 
    ...
    {  
      ...
      M5.Lcd.setTextSize(3);
      M5.Lcd.setCursor(30, 120);
      M5.Lcd.printf("No Finger!     \r\n"); 
      M5.Lcd.setTextColor(RED, BLACK);
      M5.Lcd.setTextSize(3);
      M5.Lcd.setCursor(20, 40);
      M5.Lcd.printf("BPM: NA.  \r\n");
      M5.Lcd.setTextColor(BLUE, BLACK);
      //M5.Lcd.setTextSize(3);
      M5.Lcd.setCursor(20, 80);
      M5.Lcd.printf("Spo2: NA.  \r\n");
      //Disbuff.setTextColor(RED);
